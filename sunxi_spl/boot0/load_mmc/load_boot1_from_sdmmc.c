...
__s32 load_boot1_from_sdmmc( char *buf)
{
	...
	//for(i=0;i<4;i++)
	{
		...
		/* load 1k uboot head */
	    if( mmc_bread( card_no, UBOOT_START_SECTOR_IN_SDMMC, /* 1024/512 */ 2, (void *)CONFIG_SYS_TEXT_BASE ) != /* (1024/512) */2)
		{
	    ...
	    if( mmc_bread( card_no, UBOOT_START_SECTOR_IN_SDMMC, length >> 9/* length/512 */, (void *)CONFIG_SYS_TEXT_BASE )!= (/* length/512 */length >> 9))
		{
			printf("Fail in reading uboot head.\n");
			goto __card_op_fail__;
		}
		...
		do
		{
			...
			/* load 1k uboot head */
		    if( mmc_bread( card_no, start_sector, /* 1024/512 */ 2, (void *)CONFIG_SYS_TEXT_BASE ) != /* (1024/512) */2)
			{
				printf("Fail in reading uboot head.\n");
				continue;
			}
		    ...
		    if( mmc_bread( card_no, start_sector, length >> 9/* length/512 */, (void *)CONFIG_SYS_TEXT_BASE )!= (length >> 9/* length/512 */))
			{
				printf("Fail in reading uboot head.\n");
				continue;
			}
