...
#define SPINOR_VERIFY
#ifdef SPINOR_VERIFY
int spinor_verify;
u32 spinor_verify_img(char *src, uint len_bytes);
...
#ifdef SPINOR_VERIFY
	if (!script_parser_fetch("card_boot", "spinor_verify", &spinor_verify, 1) && spinor_verify == 1)
	{
		if (spinor_verify_img(spinor_store_buffer, total_write_bytes))
			return -1;
	}
#endif
...
#ifdef SPINOR_VERIFY
#define VERIFY_READ_SIZE	(1024*1024)
u32 spinor_verify_img(char *src, uint len_bytes)
{
	char *buffer;
	int ret;
	int i;
	int start = 0;
	int remain_bytes = len_bytes;
	int once_read_bytes;

	printf("[verify] img size = %d\n", len_bytes);

	buffer = (char *)malloc(VERIFY_READ_SIZE);
	if (!buffer)
	{
		printf("error: can't malloc %s %d\n", __FILE__, __LINE__);
		return -1;
	}
	while (remain_bytes > 0)
	{
		if ((remain_bytes - VERIFY_READ_SIZE) > 0)
			once_read_bytes = VERIFY_READ_SIZE;
		else	
			once_read_bytes = remain_bytes;
		
		memset(buffer, 0xff, VERIFY_READ_SIZE);
		printf("start = 0x%x read_bytes = 0x%x\n", start, once_read_bytes);
		ret = __spinor_sector_read(start, once_read_bytes/512, buffer);
		if(ret)
		{
			printf("spinor read img fail\n");
			free(buffer);
		...
		for(i = start; i < once_read_bytes; i++)
		{
			if(buffer[i] != src[i])
			{
				printf("compare spinor read and write error\n");
		...
		printf("verify the buf ok\n");
		remain_bytes -= once_read_bytes;
		start += once_read_bytes;
	}
	free(buffer);
	printf("spinor verify img ok\n");
	return 0;
}
