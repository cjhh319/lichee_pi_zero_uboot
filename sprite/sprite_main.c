...
************************************************************************************************************
*/
static script_gpio_set_t ipc_led_gpio[1];
__u32	gpio_handle;
	
static int ipc_led_init()
{
		//init the led jpio as in/out
	int ret =-1;
	script_parser_value_type_t type;
	ret = script_parser_fetch_ex("card_product_led","card_product_led_port",(int *)&ipc_led_gpio[0],&type,sizeof(script_gpio_set_t)/sizeof(__u32));
	if(ret != 0)
	{
		printf("@@@[debug_jaosn]:get the config failed @@@@\n");
		return -1;
	}
	if(SCIRPT_PARSER_VALUE_TYPE_SINGLE_WORD == type) {
		printf("%d\n",ipc_led_gpio[0]);
	}else if(SCIRPT_PARSER_VALUE_TYPE_GPIO_WORD == type) {
		printf(" gpio=%d,mul_sel=%d,data:%d\n", ipc_led_gpio[0].port_num, ipc_led_gpio[0].mul_sel, ipc_led_gpio[0].data);
	}else if(SCIRPT_PARSER_VALUE_TYPE_STRING == type) {
		printf("%s\n",	ipc_led_gpio[0]);
	} else {
		ret = -1;
	...
	gpio_handle = gpio_request(&ipc_led_gpio[0],1);
	if(gpio_handle != 0)
	{
		printf("gpio_name=%s, gpio=%d, ret=%d\n",  ipc_led_gpio[0].gpio_name, ipc_led_gpio[0].port_num, gpio_handle);
				
	}else
	{
		printf("@@@ [debug_jaosn]:gpio request failed @@@\n");
		return -1;		
	}
	return 0;
...
static int led_set_value(int value)
{
	//1点亮led; 0 熄灭led
	gpio_write_one_pin_value(gpio_handle,value,ipc_led_gpio[0].gpio_name);
	return 0;
}
...
//		nand_get_mbr((char *)img_mbr, 16 * 1024);
		...
		tick_printf("beginsprite_cartoon_upgrade to download part\n");
		//开始烧写分区
	...
	{
		//ipc 卡量产led灯配置
		if(script_parser_fetch("card_product_led","card_product_led_used",&value,1) == 0)
		{
			printf("[debug_jaosn]:this config is for ipc\n");
            if( value != 0 )
            {
                printf("led init..............\n");
     		    ipc_led_init();
