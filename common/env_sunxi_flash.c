...
#ifdef  TFTP_ENV_DEBUG
#define TFTP_ENV_DEBUG(fmt,args...)	printf(fmt ,##args)
#else
#define TFTP_ENV_DEBUG(fmt,args...) do { } while(0)
#endif
...
static void flash_env_relocate_spec(int workmode)
{
	...
	else
	{
		start = sunxi_partition_get_offset_byname(CONFIG_SUNXI_ENV_PARTITION);
		if(!start){
			TFTP_ENV_DEBUG("fail to find part named %s\n", CONFIG_SUNXI_ENV_PARTITION);
			use_default();
			return;
		}
	...
	ret = script_parser_fetch("env_crc","env_crc_check_no_used",&value,1);
        if(value == 1)
        {
            printf("@@@@@@@[debug_jaosn]:this is for ipc @@@@@@@\n");
            env_import(buf, 0);
        }else
        {     
            //printf("@@@@@@@[debug_jaosn]:this is for cdr @@@@@@@\n");    
            env_import(buf, 1);
        }
