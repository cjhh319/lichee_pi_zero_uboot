...
#elif defined(CONFIG_ARCH_SUN9IW1P1) || defined(CONFIG_ARCH_SUN8IW7P1)
	sunxi_set_fel_flag();
...
void fastboot_partition_init(void)
{
	...
	//char part_name[16];						//modify by jason
...
static void set_boot_type_cmd(char *boot_commond)
{
	...
	{
		strcpy(boot_commond, "run setargs_spinor boot_normal");
	}
	else
	{
...
{
	int fel_key_use = 0 ;
	 script_parser_fetch("fel_key", "fel_key_used", &fel_key_use, 1);
	if(fel_key_use == 1){
		//printf("@@@@@@[debug_jaosn]:the fel_key_used is %d @@@@@@@\n");
	int ret;
	int fel_key_max;
...
************************************************************************************************************
*/
int gpio_control_flashing(void)
{
	user_gpio_set_t	gpio_init;
	int	ret, i, delay, times, used = 0;
	int  led_status = 1;
	__u32 led_hd;

	ret = script_parser_fetch("gpio_flashing", "used", &used, sizeof(int) / 4);
	if (ret || (used == 0))
	{
		return 0;
	}

	ret = script_parser_fetch("gpio_flashing", "flashing_delay", (void *)&delay, 1);
	if((ret) || (!delay))
	{
		delay = 50;
	}

	ret = script_parser_fetch("gpio_flashing", "flashing_times", (void *)&times, 1);
	if((ret) || (!delay))
	{
		times = 3;
	}

	printf("try gpio_flashing\n");
	memset(&gpio_init, 0, sizeof(user_gpio_set_t));
	//配置输出gpio口
	ret = script_parser_fetch("gpio_flashing", "flashing_gpio0", (void *)&gpio_init, sizeof(user_gpio_set_t)>>2);
	if(!ret)
	{
		if(gpio_init.port)
		{
			led_hd = gpio_request(&gpio_init, 1);
			if(!led_hd)
			{
			...
			for(i = 0; i < times; i++)
			{
				gpio_write_one_pin_value(led_hd, led_status, "flashing_gpio0");
				__msdelay(delay);
...
************************************************************************************************************
*/
#define ONEKEY_USB_RECOVERY			(0x01)
#define ONEKEY_SPRITE_RECOVERY	(0x02)
...
static void dump_data(unsigned int *buf)
{
	int i = 0;
	for ( i=0; i < 100; i++)
	{
		printf("logobuf[0x%x]=0x%x\n",buf+i*4,*(buf+i*4));
	}
}
...
//	dump_data(0x40000000);
	...
	if(!jpegdec(0x40000000,SUNXI_DISPLAY_FRAME_BUFFER_ADDR,&rgb_info))
	{
		gd->fb_base = rgb_info.buffer ;
...
#if 0	
#ifdef CONFIG_READ_LOGO_FOR_KERNEL
    sunxi_flash_read_bootlogo(0,LCD_CONFIG_MEMBASE_LOGO_CLOSE,"shutdown_logo");	
#endif
#endif
